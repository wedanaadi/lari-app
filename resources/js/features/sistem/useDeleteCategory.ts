...
interface IDeleteCategoriesFn {
  id: TItemId,
  token: string
}

const deleteCategoriesFn = async (Data: IDeleteCategoriesFn) => {
  const { data } = await axiosInstance.delete(`/categories/${Data.id}`,{
    headers: {
...
type DeleteCategoriesProps = {
  onSuccess: () => void,
}

export function useDeleteCategories({onSuccess}:DeleteCategoriesProps) {
