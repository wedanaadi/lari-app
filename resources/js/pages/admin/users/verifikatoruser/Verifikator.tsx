...
export default function Verifikator() {
  ...
  const [verifikatorUserId, setVerifikatorUserId] = useState<TItemId>("")
  ...
  const {
    data: verifikatorUser,
    isLoading: verifikatorUserIsLoading,
    isError: verifikatorUserIsError,
    refetch: verifikatorUserRefetch
  } = useFetchUser({
    aksi:'verifikator',
    ...
    onError: (err) => {
      if (err.response) {
        let message = "Gagal load data user verifikator"
  ...
  const { mutate: deleteVerifikatorUser } = useDeleteUser({
    onSuccess: () => {
      ToastNotification({
        message: "User verifikator delete Successfuly",
        type: "success",
      });
      verifikatorUserRefetch();
    },
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h1 className="sm:text-2xl font-semibold">Data Verifikator</h1>
        ...
        <div className="p-4">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-2 border-third text-center text-base font-light">
              ...
              <tbody>
                {verifikatorUserIsLoading && (
                  <tr>
                    <td colSpan={5}>loading......</td>
                  </tr>
                )}
                {
                  !verifikatorUserIsLoading && !verifikatorUserIsError && verifikatorUser.data?.data.length > 0 ? (
                    verifikatorUser.data?.data.map((row, index) => (
                      <tr
                      ...
                      >
                        <td className="whitespace-nowrap border-r border-third px-6 py-1 font-medium">
                          {index + verifikatorUser.data.from}
                        </td>
                        <td className="whitespace-nowrap border-r border-third px-6 py-1 font-medium">
                          {row.username}
                        </td>
                  ...
                  ) : !verifikatorUserIsLoading && !verifikatorUserIsError && (
                    <tr>
            ...
            {verifikatorUser ? (
              <Pagination
                ...
                currentPage={currentPage}
                totalCount={verifikatorUser.data.total}
                pageSize={verifikatorUser.data.per_page}
